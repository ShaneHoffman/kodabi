...
function report(manifest) {
  ...
  console.log(
    [
      ...
      "  pnpm tauri dev",
      "",
      "One variable, not three. It derives the vault, the index, the ledger, the",
      "config dir and the WebView2 profile from this base, so the vault and its",
      "index can never be pointed at different places — which used to be the",
      "destructive way to get this wrong. KODABI_KB_ROOT, KODABI_INDEX_DB or",
      "KODABI_LEDGER_DB set alongside it are refused at startup rather than",
      "half-honoured.",
      "",
