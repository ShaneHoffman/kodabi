 ...
 *
 * `prefers-contrast: more` is honoured too, but that is the OS setting, and on
 * Windows it is reached only by turning on a Contrast theme — which also flips
 ...
 *
 * The two branches are OR-ed, so the switch can only ever ADD contrast: it
 * cannot overrule an OS request for more. See docs/DESIGN_SYSTEM.md §6.
 *
...
/** The OS request, which is the other way into high contrast. Grove's `.hc`
 * block is plain CSS with no query of its own, so the OS branch has to be read
 * here and folded into the class. Module scope so the listener attaches once
 * per window. */
const prefersMoreContrast =
  typeof window !== "undefined" && typeof window.matchMedia === "function"
    ? window.matchMedia("(prefers-contrast: more)")
    : null;
/** The one value storage ever holds. `more` rather than `on` because it mirrors
 * the media feature's own vocabulary — a second word for one state is how a
 * stored value and the code that reads it drift apart. */
const VALUE = "more";
 ...
 * it. Must target documentElement, not body — the `.hc` block keys off :root.
 *
 * Only the in-app toggle is persisted; the OS request is read live and OR-ed
 * in, which is what keeps the switch additive — it can add contrast, never take
 * it away. */
