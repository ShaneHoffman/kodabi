...
const LIST_OUTSTANDING_ITEMS_DESCRIPTION: &str = "List action items that are not done (open/overdue), extracted from meetings and chats and linked to their source note. Filter by project subtree, owner, status, due-before date, or source note.";
