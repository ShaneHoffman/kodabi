...
/// The engine windows each `accept` down to the VAD's 512-sample granularity
/// internally (see `silero::feed_windowed`) and finalised segments free their
/// VAD buffer space as they are queued, so chunk size no longer risks
/// overflowing the ~30 s circular buffer — chunking here just keeps the
/// finalised-segment queue drained at a realistic streaming cadence.
