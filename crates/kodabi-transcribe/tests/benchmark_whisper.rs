//! Runs whisper.cpp (VAD-gated) over the real-meeting benchmark fixture and
//! scores it against the reference transcript — the "Benchmark both
//! engines" step of FOUNDING_DOC §3.4/§7 ("Default STT engine"). See
...
//! cargo test -p kodabi-transcribe --features whisper -- --ignored --nocapture benchmark
//! ```
...
/// Transcribes one channel's samples through a fresh VAD-gated engine
/// (avoids VAD state bleeding between channels), feeding it in [`CHUNK_SAMPLES`]
/// chunks and draining after each. Times only the transcription work — model
/// load is excluded from the reported real-time factor.
fn transcribe_channel(samples: &[f32], glossary_terms: &[String]) -> (Vec<Segment>, u128) {
    let mut engine =
        whisper_with_vad(whisper_config(), vad_config()).expect("whisper engine should load");
    engine
        .set_bias(glossary_terms)
        .expect("set_bias applies the glossary as whisper's initial prompt");
...
#[test]
#[ignore = "requires the local benchmark fixture (KODABI_BENCHMARK_MEETING_DIR) and real whisper.cpp + Silero VAD model files (WHISPER_MODEL, VAD_MODEL env vars)"]
fn benchmark_whisper_against_the_reference_transcript() {
    ...
    let hyp_path =
        write_hypothesis(&dir, "whisper", &hypothesis).expect("hypothesis file should write");
