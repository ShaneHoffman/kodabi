...
2. **Record it** through the real capture pipeline:
   ```sh
   cargo run -p kodabi-audio --example record_meeting -- ./out/meeting
   ```
...
4. **Transcribe each channel separately and hand-check every proper noun** against the audio —
   ...
   there. Merge the two channels' segments by `start_ms` (as `kodabi_core::raw_session::assemble`
...
- `reference.jsonl` — the transcript-of-record: newline-delimited `TranscriptSegment` JSON, one
  object per line, matching `crates/kodabi-core/src/raw_session.rs` and
...
- `glossary.yml` — proper nouns present, in the project-glossary shape
  (`crates/kodabi-core/src/glossary.rs`):
  ```yaml
  terms:
    - term: OKIES
      definition: ""
...
harness should read a single env var — e.g. `KODABI_BENCHMARK_MEETING_DIR` — pointing at this
