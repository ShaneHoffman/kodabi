...
//!
//! ```text
//! WHISPER_MODEL=... cargo test -p kodabi-transcribe --features whisper-cuda -- --ignored
//! ```
//!
...
use kodabi_transcribe::{WhisperConfig, WhisperEngine};
...
fn accepts_an_initial_prompt() {
    let mut engine = WhisperEngine::new(real_config()).expect("engine should load");
    engine
        .set_bias(&["OKIES".to_owned(), "ForeUp".to_owned()])
