...
mod tests {
    ...
    fn interior_nul_bytes_are_stripped() {
        ...
        assert_eq!(build_bias_prompt(&terms), Some("MERIDIAN".to_owned()));
    }

    #[test]
    fn a_term_that_is_only_a_nul_byte_is_dropped() {
        let terms = ["\0".to_owned(), "TeeTrack".to_owned()];
        assert_eq!(build_bias_prompt(&terms), Some("TeeTrack".to_owned()));
    }
}
