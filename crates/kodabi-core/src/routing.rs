...
fn score_candidate(title: &[String], body: &[String], candidate: &ProjectSignals) -> ProjectScore {
    ...
    // One signal per distinct path segment: "Paradise Golf" is a single
...
mod tests {
    ...
    /// a term-rich `Paradise Golf` and a smaller `Growth/Q3`.
    ...
    fn multi_word_and_hyphenated_phrases_match() {
        let candidates = [signals(
            "Paradise Golf",
            &[("tee sheet", &[]), ("ForeUp", &["4up"])],
        )];

        let scores = score_projects(body("the Tee-Sheet import from 4Up"), &candidates);
        let score = score_of(&scores, "Paradise Golf");
    ...
    fn repeat_occurrences_and_aliases_count_one_entry_once() {
        let candidates = [signals("Paradise Golf", &[("ForeUp", &["4up"])])];
        let scores = score_projects(body("ForeUp aka 4up; ForeUp again"), &candidates);
        let score = score_of(&scores, "Paradise Golf");
    ...
    fn title_hits_weigh_double_and_do_not_stack_with_body() {
        let candidates = [signals("Paradise Golf", &[("GreenFlow", &[])])];
    ...
    fn name_segment_hits_score_name_weight() {
        ...
        let candidates = [signals("Paradise Golf", &[])];
        let scores = score_projects(body("mentioned Paradise Golf in passing"), &candidates);
        let score = score_of(&scores, "Paradise Golf");
        assert_eq!(score.name_hits, 1);
        assert_eq!(score.weight, NAME_WEIGHT);
        // …so half of it alone is no hit.
        let scores = score_projects(body("paradise island"), &candidates);
        assert_eq!(score_of(&scores, "Paradise Golf").weight, 0.0);
    ...
    fn worked_examples_produce_exact_confidences() {
        ...
        let contested = "OKIES kickoff with ForeUp on the tee sheet; timeline follows OKR.";
        ...
        let routing = route(
            titled("Paradise Golf weekly sync", "GreenFlow status is nominal."),
            &candidates,
            &config,
        );
        assert_eq!(
    ...
    fn split_evidence_routes_to_inbox_with_low_recorded_confidence() {
        ...
        let tied = "OKIES and ForeUp progress blocked on OKR activation review.";
    ...
    fn a_bare_body_name_drop_is_not_enough_to_file() {
        ...
        let routing = route(
            body("mentioned Paradise Golf in passing"),
            &fixture(),
    ...
    fn confidence_exactly_at_threshold_routes() {
        ...
        let three_terms = "OKIES update, ForeUp demo, GreenFlow next";
    ...
    fn tie_routes_to_inbox_and_score_order_is_deterministic() {
        ...
        assert_eq!(scores[1].project, "Paradise Golf");
    ...
    fn invalid_candidate_slug_falls_back_to_inbox() {
        ...
        let candidates = [signals(
            "con",
            &[("OKIES", &[]), ("ForeUp", &[]), ("GreenFlow", &[])],
        )];
        let routing = route(
            body("OKIES update, ForeUp demo, GreenFlow next"),
            &candidates,
            &RoutingConfig::default(),
        );
        assert_eq!(routing.project(), INBOX);
    ...
    fn invalid_threshold_falls_back_to_default() {
        ...
        let strong = "OKIES rollout: ForeUp sync for the tee sheet, GreenFlow irrigation checks.";
        let weak = "mentioned Paradise Golf in passing";
    ...
    fn discover_projects_finds_nested_projects_sorted() {
        ...
        for dir in ["Paradise Golf", "Growth/Q3", "Data/raw"] {
        ...
        assert_eq!(
            projects,
            ["Data", "Data/raw", "Growth", "Growth/Q3", "Paradise Golf"]
        );
    }
    ...
    fn load_project_signals_pairs_projects_with_glossaries() {
        let vault = tempdir().unwrap();
        let golf_dir = vault.path().join("Paradise Golf");
        ...
        assert_eq!(signals[1].project, "Paradise Golf");
    ...
    fn high_confidence_note_lands_in_its_project_folder() {
        ...
        let text =
            body("OKIES rollout: ForeUp sync for the tee sheet, GreenFlow irrigation checks.");

        let (routing, path) = write_routed(vault.path(), text, Some("OKIES rollout"));

        assert_eq!(
            path,
            vault.path().join("Paradise Golf").join("okies-rollout.md")
        );
    ...
    fn low_confidence_note_lands_in_inbox_with_recorded_score() {
        let vault = fixture_vault();
        let text = body("OKIES kickoff with ForeUp on the tee sheet; timeline follows OKR.");
