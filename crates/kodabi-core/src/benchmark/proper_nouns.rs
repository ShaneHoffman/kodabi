...
mod tests {
    ...
    fn partial_recall_is_hits_over_reference_occurrences() {
        let glossary = glossary_with("MERIDIAN", &[]);
        let reference = vec![
            segment(0, Channel::You, "talk about MERIDIAN today"),
            segment(1, Channel::You, "MERIDIAN again"),
        ];
        let hypothesis = vec![segment(0, Channel::You, "talk about MERIDIAN today")];
    ...
    fn alias_hit_folds_into_the_canonical_term() {
        let glossary = glossary_with("TeeTrack", &["t-track"]);
        let reference = vec![segment(0, Channel::You, "we use TeeTrack daily")];
        // Mistranscribed as the alias spelling.
        let hypothesis = vec![segment(0, Channel::You, "we use t-track daily")];
    ...
    fn hyphenation_variant_only_matches_when_enumerated_as_an_alias() {
        let reference = vec![segment(0, Channel::You, "TeeTrack is the vendor")];
        let hypothesis = vec![segment(0, Channel::You, "Tee-Track is the vendor")];

        let without_alias = glossary_with("TeeTrack", &[]);
        let with_alias = glossary_with("TeeTrack", &["Tee-Track"]);
    ...
    fn over_emission_is_capped_at_full_recall() {
        let glossary = glossary_with("MERIDIAN", &[]);
        let reference = vec![segment(0, Channel::You, "MERIDIAN")];
        let hypothesis = vec![segment(0, Channel::You, "MERIDIAN MERIDIAN MERIDIAN")];
