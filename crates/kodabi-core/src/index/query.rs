...
impl NoteIndex {
    ...
    /// updates the same row (its `pk` is preserved, keeping the FTS and vector
    /// entries valid). The FTS index is maintained by the schema's triggers; the
    /// tag set is refreshed explicitly. All of it is one transaction, so a
    /// partially-applied upsert is impossible.
    ...
    /// Loads a note's tags, sorted for deterministic output.
