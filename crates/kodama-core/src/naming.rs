...
mod tests {
    ...
    fn two_devices_same_instant_produce_different_filenames() {
        ...
        let name_a = session_filename(when, &a, Some("paradise golf sync"), "jsonl");
        let name_b = session_filename(when, &b, Some("paradise golf sync"), "jsonl");
    ...
    fn filename_matches_scheme_and_stays_short() {
        ...
        let name = session_filename(instant(123), &device, Some("Paradise Golf Sync!"), "jsonl");

        assert_eq!(
            name,
            "20260712T140335123Z-k4m2xp7q-paradise-golf-sync.jsonl"
        );
    ...
    fn slugify_sanitizes_case_punctuation_and_length() {
        assert_eq!(slugify("Paradise Golf Sync!"), "paradise-golf-sync");
    ...
    fn parse_session_filename_round_trips() {
        ...
        assert_eq!(parsed.slug.as_deref(), Some("paradise-golf-sync"));
