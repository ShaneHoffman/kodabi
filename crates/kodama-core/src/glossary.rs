...
impl Glossary {
    ...
    /// Without this a hand-authored file with two `OKIES` entries would parse
...
mod tests {
    ...
    fn save_then_load_round_trips() {
        ...
        glossary
            .upsert(
                term("ForeUp", "Tee-sheet / POS vendor.", &["4up"]),
                OnConflict::Error,
            )
            .unwrap();
...
terms:
  - term: OKIES
    definition: Oklahoma migration project.
    aliases: [okies, oh-kies]
  - term: ForeUp
...
"#;
        ...
        assert_eq!(
            loaded.get("okies").unwrap().definition,
            "Oklahoma migration project."
        );
        // No `aliases` key in the hand-written entry should default to empty.
        assert!(loaded.get("foreup").unwrap().aliases.is_empty());
    }
    ...
    fn upsert_creates_new_term() {
        ...
        let created = glossary
            .upsert(
                term("OKIES", "Oklahoma migration project.", &[]),
                OnConflict::Error,
            )
            .unwrap();
        assert!(created);
    ...
    fn upsert_updates_existing_term_case_insensitively() {
        let mut glossary = Glossary::default();
        glossary
            .upsert(term("OKIES", "First definition.", &[]), OnConflict::Error)
            .unwrap();

        let created = glossary
            .upsert(
                term("okies", "Updated definition.", &["okies-alias"]),
                OnConflict::Update,
            )
            .unwrap();
        ...
        assert_eq!(glossary.terms().len(), 1);
        assert_eq!(
            glossary.get("OKIES").unwrap().definition,
            "Updated definition."
        );
    }
    ...
    fn upsert_on_conflict_error_rejects_existing_term() {
        ...
        let result = glossary.upsert(term("okies", "Second definition.", &[]), OnConflict::Error);

        assert!(matches!(result, Err(GlossaryError::Conflict { term }) if term == "okies"));
        assert_eq!(glossary.terms().len(), 1);
    }
    ...
    fn get_is_case_insensitive_and_trims() {
        ...
        assert!(glossary.get("  foreup ").is_some());
        assert!(glossary.get("FOREUP").is_some());
    ...
    fn separate_project_dirs_stay_isolated() {
        ...
        glossary_a
            .upsert(term("OKIES", "Client A's project.", &[]), OnConflict::Error)
            .unwrap();
        glossary_a.save(dir_a.path()).unwrap();

        let mut glossary_b = Glossary::default();
        glossary_b
            .upsert(term("ForeUp", "Client B's vendor.", &[]), OnConflict::Error)
            .unwrap();
        ...
        assert!(loaded_a.get("okies").is_some());
        assert!(loaded_a.get("foreup").is_none());
        assert!(loaded_b.get("foreup").is_some());
        assert!(loaded_b.get("okies").is_none());
    }
    ...
    fn get_resolves_aliases() {
        ...
        glossary
            .upsert(
                term("ForeUp", "Tee-sheet / POS vendor.", &["4up", "fore-up"]),
                OnConflict::Error,
            )
            .unwrap();
        ...
        assert_eq!(glossary.get("4up").unwrap().term, "ForeUp");
        assert_eq!(glossary.get("  Fore-Up ").unwrap().term, "ForeUp");
    ...
    fn upsert_trims_term_and_aliases_and_drops_blanks() {
        ...
        assert_eq!(stored.term, "OKIES");
        assert_eq!(stored.aliases, vec!["okies".to_string()]);
    }
...
"#;
        ...
        assert!(matches!(result, Err(GlossaryError::Duplicate { term, .. }) if term == "okies"));
    }
