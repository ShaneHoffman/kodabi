  ...
  "$defs": {
    ...
    "Project": {
      ...
      "properties": {
        ...
        "display_name": { "type": "string", "description": "Human-facing project name (the leaf, e.g. \"Q3\" or \"Paradise Golf\")." },
...
The Phase 3 milestone — *"What's outstanding on Paradise Golf?" answered correctly in-app from real
meeting history* — traces through this surface as:

1. `list_projects()` → resolve the free-text name "Paradise Golf" to its slug (e.g. `"Paradise Golf"`
   or a nested slug if it's a sub-project).
2. `list_outstanding_items(project: "Paradise Golf")` or `get_project_context(project: "Paradise Golf", include_outstanding: true)` → the not-done action items for that project, each carrying a `source` (`NoteRef`) back to its meeting.
