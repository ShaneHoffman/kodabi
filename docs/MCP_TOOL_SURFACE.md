...
```json
{
  "$defs": {
    ...
    "MeetingCategory": {
      "type": "string",
      "enum": ["standup", "one-on-one", "client", "working-session", "review", "all-hands", "observer"],
      "description": "The meeting's genre: the second facet beside `type`, classified at distill time and correctable by hand."
    },
    "Channel": {
      "type": "string",
    ...
    "NoteSummary": {
      ...
      "properties": {
        ...
        "source": { "type": "string", "description": "Frontmatter source: a capture keyword (transcript | quick-capture | chat | import | manual | digest) or a repo-relative path to the raw artifact. Disambiguation: a value exactly equal to a keyword is that keyword; anything else is the path." },
        ...
        "category": { "oneOf": [ { "$ref": "#/$defs/MeetingCategory" }, { "type": "null" } ], "description": "The meeting's genre, or null when the note carries none (an unclassified meeting, or any non-meeting note, where the key is invalid)." },
        "category_confidence": { "type": ["number", "null"], "minimum": 0, "maximum": 1, "description": "How strongly the distill pass backed `category`, or null. Cleared when a person sets the category by hand, so its presence answers whether anyone confirmed the genre." },
        "tracking": { "type": ["string", "null"], "enum": ["tracked", "context-only", null], "description": "The per-meeting commitment-tracking override, or null when the note inherits (the category default, then the global default of full tracking). Gates which extracted items earn a ledger entry; extraction itself is unconditional." }
      },
