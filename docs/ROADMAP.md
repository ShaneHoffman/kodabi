...
- [x] Commitment triage: a review moment for a heavy day, and a firmness signal so less of it needs reviewing. The whole-vault Commitments view opens with a strip naming what enrolled since you last looked, grouped by the meeting that produced it, with Keep and Untrack per row and batched untrack/snooze for a whole group — **deliberately not an approval gate**, since an uncleared inbox goes stale and takes the ledger's credibility with it. Underneath, the distill pass now classifies each action item firm or soft, and a soft one ("we should probably look into that sometime") extracts into the note carrying a `(tentative)` marker but never enrols (#245 `feat/commitment-triage`)
- [x] The completion moment: clearing something you owe now *lands*. The check draws on, the card rests a beat fully stated, then leaves under the same Vanish the Inbox uses — optimistically, so the motion never waits on the write, while the ledger still gets the last word and a failed write travels the row back. It arrives on a settled shelf that leads with the week's wins rather than reading as an archive, and an entry an evidence pass closed on its own says so in the active voice, dated from the claim that closed it. The dock's Commitments row carries the count of what is still on you — Mine only, since watching your own number come down is the point (#246 `feat/completion-that-lands`)
...
- [x] Ledger over MCP: coherent commitments reads plus a mark-done write tool, so chat and the Commitments view can never disagree — `list_commitments` renders the same core join the Commitments view does, `update_action_item` writes the checkbox and the closure together, and the sidecar reaches the ledger through a third sandbox-derived path (#241)
- [x] Waive everywhere: the verb that was cut from the MCP surface for lack of a note-side write now has one. A waive appends a date-only `- Waived <date>.` line under the item and never ticks the box (waived is not done), in the app and over the new `waive_action_item` tool alike — so the note is a truthful record of the verdict, and the file write is what carries a chat-made waive into an open window, the one route a process with no `AppHandle` has. Reopening leaves the line: annotate, never destroy (#249 `feat/waive-everywhere`)
