...
| code-review | Skip → superseded | Originally skipped as a shadow of the built-in `/code-review high` the column ran. The column now runs Kodabi's own `.claude/skills/code-review-fix/` (review → fix → gates → commit), which the built-in cannot do: it isn't invocable from inside a session, and it never fixes or commits. |
