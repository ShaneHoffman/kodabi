...
- **Fallback engine: whisper.cpp (large-v3-turbo)** for multilingual needs and the strongest glossary-biasing mechanism (initial prompt) — selected at build time in place of Parakeet, not a runtime fallback (the two link modes can't coexist in one binary). Must be paired with **Silero VAD** to chop silence before it hallucinates. **Deferred for v1:** that mandatory VAD path is far too slow — measured at 0.05x realtime against Parakeet's 9.94x on the same fixture, roughly 200x slower — so Parakeet is the sole shipping engine. (The Windows sherpa-onnx/ONNX Runtime API-version crash that originally blocked the path, board task #53, has since been fixed; the measurement it unblocked is what now carries the decision. Both recorded in `docs/RESOURCE_BUDGET.md`.)
...
- **Capture is never invisible.** There is no state of the app in which audio capture is running
  ...
  designed in §4.
- **Retention policy:** governs the stored transcript — optional "distill then discard the raw transcript after N days." Raw client-call transcripts accumulating forever is a liability. **No audio is *retained* in v1** — only the transcript + timestamps become a lasting on-disk artifact, so there is no retained audio for the retention policy to prune yet. (The task #57 incremental-capture spool flushes audio to disk *transiently* during a meeting for crash recovery, but that spool is cleared once the session distills, and an orphaned spool left by a crash is reclaimed on the next startup — not by retention.) When an opt-in audio-retention toggle is eventually pulled by a use case, the retention policy must cover it too. **One gap the in-app policy cannot prune directly, but does close:** the `claude` CLI keeps its *own* Claude Code session logs (under `~/.claude`), which would otherwise contain the transcript text passed to it — outside our retention control. Both places Kodabi spawns `claude` disable that logging by setting `CLAUDE_CODE_SKIP_PROMPT_HISTORY` on the child process: the headless distill runner (`kodabi-llm`) and the embedded terminal (`src-tauri/src/terminal_cmds.rs`, which hosts an interactive `claude` for chat over the KB, §4). So neither transcript text nor chat lingers there, and the env var is scoped to Kodabi's own spawns — it never touches the user's global Claude Code config or their other sessions. **Scope carve-out — `chats/` is deliberately outside the retention sweep.** The policy governs `<KB>/sessions/`: a meeting transcript is a lossy machine artifact of an event that happened in a room, and discarding it once distilled is the point. A chat transcript (`<KB>/chats/*.jsonl`, §3.6's "chats are documents too") is text the user wrote and text they read, and because the same `CLAUDE_CODE_SKIP_PROMPT_HISTORY` above means Claude Code keeps no copy, it is the *only* record that conversation ever existed. Discarding the primary and keeping the summary is the wrong direction for a knowledge tool, so a distilled chat's `source: chats/…` stays resolvable indefinitely. If an aging policy for chats is ever wanted, it is new surface, not a widening of this one.
...
- Retention setting, consent nudge, and the capture-visibility indicators the §3.7 invariant requires (in-window listening indicator, capturing-vs-idle tray icon, capture-start toast, full-screen overlay pill)
...
| ~~Crash reporting~~ | — | **DECIDED: none in v1** (2026-08-14 — the app captures no crash data at all today: no panic hook, no crash log, and a windowed release discards the `eprintln!` diagnostics, so the gap is that nothing exists to report rather than data withheld. Standing policy if ever added: strictly opt-in, local capture first, sharing a manual act, no third-party service, and no automatic transmission of user-derived content — the updater's unasked-but-content-free carve-out does not extend to crash data. See `docs/decisions/crash-reporting.md`.) | ✅ Closed |
