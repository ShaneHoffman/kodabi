...
2. A **single headless-Claude distill call** returns summary, action items, decisions, and open questions as one structured result. Transcripts over the input character budget are chunked on segment boundaries and map-reduced — one call per chunk plus a merge call returning the same single JSON shape — so a long meeting distills rather than erroring (landed as task #59, `feat/distill-token-budget`; a failure in any pass writes no note, so the session stays retryable).
